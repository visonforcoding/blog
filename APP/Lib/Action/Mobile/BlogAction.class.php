 ...
 $Model = new Model();
        if (!empty($category)) {
              $querySql = "SELECT blog.*,blog.id as blog_id,blog_cat.*,count(comm.id) "
                    ...
                    . " WHERE ( blog.cat_id = blog_cat.id AND blog_cat.category = '$category'  )"
        ...
        foreach ($res_blog as $key => $value) {
            foreach ($value as $k => $v) {
                ...
                if ($k == 'content') {
                    ...
                    $res_blog[$key]['content'] = mb_substr($content, 0, 300, 'utf-8');
                }
            }
        }
        $show = $page->display();  //显示分页导航
        $this->show = $show;
        $this->right_part();  //获取右侧栏内容  获取博文分类
    ...
    public function blogRead() { 
        ...
        if(!empty($email)){
            //评论
            $this->addComment($id, $nick,$email,$comment);
            //发邮件提醒
            $address = array(
                array(
        ...
        /* * * 遍历评论* */
        $res_comment = $comment_tab->where("blog_id = '{$id}'")
                                   ->select();
        //更新阅读次数
